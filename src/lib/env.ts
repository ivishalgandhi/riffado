...
export const envSchema = z.object({
    ...
    // random valid proxy from the operator's Webshare list, with
    // automatic rotation on 403/407. Unset (default) keeps every call
    // on the direct egress path — the right default for self-hosters
    // on residential or homelab IPs. Operators whose VPS egress is not
    // viable for direct Plaud calls set this to route via their own
    // Webshare datacenter proxies. See src/lib/plaud/proxy.ts.
    WEBSHARE_API_KEY: z
        .string()
        .optional()
        .transform((val) => (val === "" ? undefined : val)),
